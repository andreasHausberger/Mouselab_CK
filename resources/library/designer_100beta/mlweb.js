...
function ExpMatrix(M) { // expand data matrices
    var Mrows = M.split("`");
...
function ExpRow(M) { // expand data vectors
...
function btnHover(loc, act) {
    if (act == 'out') {
        if (loc.className.indexOf(" ") > 0) {
            tempstyle = loc.className.substring(0, loc.className.indexOf(" "));
        }
    }
    else {
...
function checkForm(formHandle) {
    if (chkFrm) {
        ...
        for (i = 0; i < noElm; i++) {
            ...
            ;
            if (elemHandle.type == "select-one") {
                if (elemHandle.options[elemHandle.selectedIndex].value == "") {
                    filled = false;
                    break
                }
                ;
            ...
            if (elemHandle.type == "radio")   // procedure to check radio buttons
            {
                ...
                ;  // if checked button found; continue
        ...
        if (!filled) {
            alert(warningTxt);
            timefunction('submit', 'submit', 'failed');
            return false
        }
        ;
    }

    if ((chkchoice == "nobuttons") | !chkFrm) {
        return true;
    }
    if (chkchoice == true) {
...
function ShowCont(fieldname, objEvent) {
    if (!loaded) {
        return;
    } // do not open boxes when page is loading
    if (!tmDirectStart & tmActive & tmCurTime == 0) {
        startTmBar();
    }
    if (tmTimeUp) {
        return;
...
// this is to prevent enters from generating events when in click (rather than mouseover) mode
    ...
    if (objEvent.srcElement) {
        if (objEvent.srcElement.nodeName == "A") {
            return
        }
    }
    else if (objEvent.target) {
        if (objEvent.target.nodeName == "A") {
            return
        }
    }
...
// check if open cell should be recorded
    if ((statecont[RowOut[row]][ColOut[col]] == "0") & !(recOpenCells)) {
        return;
    }
    if (boxOpen) {
        return;
...
// delay
    ...
    for (var i = 0; i < Dlist.length; i++) {
        if (tagcont[RowOut[row]][ColOut[col]] == Dlist[i]) {
            currCell = i;
            break;
        }
    }
    ...
    if ((prevCell != -1) & (currCell != -1)) {
        dtime = DTimes[currCell][prevCell];
    } else {
        dtime = 0
    }
    ;
...
function HideCont(fieldname, objEvent) {
    ...
    if (!boxOpen) {
        return;
    } // do not close boxes that are not open...
...
// save procesdata
    ...
    HandleTxt.style.visibility = 'hidden';
    HandleBox.style.visibility = 'visible';
}
...
function recChoice(eventname, name, value) {
    ...
    if (document.forms[mlweb_fname].choice) {
        document.forms[mlweb_fname].choice.value = name;
    }
...
// new in version 99.2: CB preset matrix
    ...
    for (j = 0; j < RowOut.length; j++) {
        for (i = 0; i < ColOut.length; i++) {
            if (statecont[j][i] == "1") {
                Dlist[Dlist.length] = tagcont[j][i];
            }
        }
    }
}
...
// if referer present (or other php/asp code) then get current hit number
    ...
    if (isNaN(subjnr) | randomOrder) {
        subjnr = -1;
        subjtype = "random";
    }

    if (document.forms[mlweb_fname].condnum) {
        document.forms[mlweb_fname].condnum.value = subjnr;
    }
    if (document.forms[mlweb_fname].expname) {
        document.forms[mlweb_fname].expname.value = expname;
    }
    if (document.forms[mlweb_fname].nextURL) {
        document.forms[mlweb_fname].nextURL.value = nextURL;
    }
    if (document.forms[mlweb_fname].subject) {
        document.forms[mlweb_fname].subject.value = subject;
    }
    if (document.forms[mlweb_fname].to_email) {
        document.forms[mlweb_fname].to_email.value = to_email;
    }


    if (CBpreset) {
        if (subjnr == -1) {
            subjnr = Math.floor(Math.random() * CBorder.length)
        }
...
// subjDen is the denominator used to devide the subj number for each counterbalance step
        ...
        if (subjtype != "random") {
            subjDen = Math.floor(subjDen * masterCond)
        }
        ;
// first determine column and row connects and switch on that
        ...
        if (subjnr == -1) {
            subjnr = Math.floor(Math.random() * numCond)
        }
//alert("total cond:" + numCond+"\nsubject: "+subjnr);

// counterbalance col groups		
        if (c1.length > 0) {
            c1_order = CountBal(subjnr / subjDen + 1, c1.length);
...
// counterbalance rows					
        if (r1.length > 0) {
            r1_order = CountBal(subjnr / subjDen + 1, r1.length);
            subjDen = subjDen * fac(r1.length);
        }
...
// reorder and resize table content
    for (j = 0; j < RowOut.length; j++) {
        for (i = 0; i < ColOut.length; i++) {
            ...
            if (txtcont[RowOut[j]][ColOut[i]].replace(/[\x20]/gi, "") == "") {
                HandleTD.innerHTML = "&nbsp;"
            }
            else {
                ...
                if (statecont[RowOut[j]][ColOut[i]] == "0") {
                    ...
                    HandleTD.innerHTML = txtcont[RowOut[j]][ColOut[i]]
                }
                ;
            }
            ;
            ...
            if (boxcont[RowOut[j]][ColOut[i]].replace(/[\x20]/gi, "") == "") {
                HandleTDbox.innerHTML = "&nbsp;"
            }
            else {
                if (colFix) {
                    tempcol = i
            ...
            if (statecont[RowOut[j]][ColOut[i]] == "1") {
                HandleTD.className = activeClass;
            } else {
                HandleTD.className = inactiveClass
            }
            ;
            ...
            if (statecont[RowOut[j]][ColOut[i]] == '0') {
                HandleBox.style.visibility = "hidden";
                HandleTxt.style.visibility = "visible";
            } else {
                HandleBox.style.visibility = "visible";
                HandleTxt.style.visibility = "hidden";
            }
    ...
    if (btnFlg > 0) {
        ...
        for (bc = 0; bc < btnTxt.length; bc++) {
            ...
            if (bc == 0) {
                defTDcolor = HandleTD.style.backgroundColor;
            }
            docstr = btn_inner[bc];
...
function initTmBar() {
    ...
    if (tmFill) {
        HandleTmBar.style.width = "0px";
        HandleTmTime.innerHTML = "0 sec";
    }
    else {
        HandleTmBar.style.width = parseInt(tmWidthPx) + "px";
        HandleTmTime.innerHTML = parseInt(tmTotalSec) + " sec";
    }
    if (tmShowTime) {
        HandleTmTime.style.visibility = "visible";
    }
    else {
        HandleTmTime.style.visibility = "hidden";
    }
}
...
function refreshTmBar() {
    ...
    if (tmFill) {
        ...
        if (tmMinLabel == "false" | tmCurTime < 60000) {
            HandleTmTime.innerHTML = parseInt(Math.round(tmCurTime / 1000)) + " " + tmSecLabel
        }
        else {
        ...
        HandleTmBar.style.width = parseInt(tmWidthPx - Math.round(tmCurTime / (tmTotalSec * 1000) * tmWidthPx)) + "px";
        HandleTmTime.innerHTML = parseInt(tmTotalSec - Math.round(tmCurTime / 1000)) + " sec";
        if (tmMinLabel == "false" | tmCurTime < 60000) {
            HandleTmTime.innerHTML = parseInt(tmTotalSec - Math.round(tmCurTime / 1000)) + " " + tmSecLabel
        }
        else {
